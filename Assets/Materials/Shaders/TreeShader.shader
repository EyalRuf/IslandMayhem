Shader "Custom/TreeShader"
{
    Properties
    {
        ...
        [Header(Wind settings)]_wind_dir("Wind Direction", Vector) = (0.5,0.05,0.5,0)
        ...
        _r_influence("Red Vertex Influence", float) = 1
        _b_influence("Blue Vertex Influence", float) = 1
    }
        ...
        SubShader
        {
            ...
            Varyings LitPassVertex(Attributes input)
            {
                ...
                float3 worldPos = mul(unity_ObjectToWorld, vertexInput.positionWS).xyz;
                float3 vertexPos = vertexInput.positionWS;

                //Tree Movement and Wiggle
                float3 offset = float3(0, 0, 0);

                //Tree Movement and Wiggle
                offset.x += (cos(_Time.z * _tree_sway_speed + (worldPos.x / _wind_size) + (sin(_Time.z * _tree_sway_stutter * _tree_sway_speed + (worldPos.x / _wind_size)) * _tree_sway_stutter_influence)) + 1) / 2 * _tree_sway_disp * _wind_dir.x * (offset.y / 10) +
                    cos(_Time.w * offset.x * _leaves_wiggle_speed + (worldPos.x / _wind_size)) * _leaves_wiggle_disp * _wind_dir.x * _b_influence;

                offset.z += (cos(_Time.z * _tree_sway_speed + (worldPos.z / _wind_size) + (sin(_Time.z * _tree_sway_stutter * _tree_sway_speed + (worldPos.z / _wind_size)) * _tree_sway_stutter_influence)) + 1) / 2 * _tree_sway_disp * _wind_dir.z * (offset.y / 10) +
                    cos(_Time.w * offset.z * _leaves_wiggle_speed + (worldPos.x / _wind_size)) * _leaves_wiggle_disp * _wind_dir.z * _b_influence;

                offset.y += cos(_Time.z * _tree_sway_speed + (worldPos.z / _wind_size)) * _tree_sway_disp * _wind_dir.y * (offset.y / 10);

                //Branches Movement
                offset.y += sin(_Time.w * _tree_sway_speed + _wind_dir.x + (worldPos.z / _wind_size)) * _branches_disp * _r_influence;

                //add offset
                vertexInput = GetVertexPositionInputs(input.positionOS.xyz + offset);
