...
public abstract class CustomNetworkTransformBase : NetworkBehaviour
{
    ...
    public float positionInterpolationSpeed = 0.1f;
    public float rotationInterpolationSpeed = 0.1f;
    public float scaleInterpolationSpeed = 0.1f;
    ...
    static Vector3 InterpolatePosition(DataPoint start, DataPoint goal, Vector3 currentPosition, float interpolationSpeed)
    {
        if (start != null)
        {
            ...
            return Vector3.MoveTowards(currentPosition, goal.localPosition, speed * Time.deltaTime * interpolationSpeed);
        }
        return currentPosition;
    }

    static Quaternion InterpolateRotation(DataPoint start, DataPoint goal, Quaternion defaultRotation, float interpolationSpeed)
    {
        if (start != null)
        {
            float t = CurrentInterpolationFactor(start, goal);
            return Quaternion.Slerp(start.localRotation, goal.localRotation, t * interpolationSpeed);
        }
        return defaultRotation;
    }

    static Vector3 InterpolateScale(DataPoint start, DataPoint goal, Vector3 currentScale, float interpolationSpeed)
    {
        if (start != null)
        {
            float t = CurrentInterpolationFactor(start, goal);
            return Vector3.Lerp(start.localScale, goal.localScale, t * interpolationSpeed);
        }
        return currentScale;
    }
        ...
        if (isClient)
        {
            ...
            if (!IsClientWithAuthority)
            {
                ...
                if (goal != null)
                {
                    ...
                    {
                        // local position/rotation for VR support
                        ApplyPositionRotationScale(InterpolatePosition(start, goal, targetComponent.transform.localPosition, positionInterpolationSpeed),
                                                   InterpolateRotation(start, goal, targetComponent.transform.localRotation, rotationInterpolationSpeed),
                                                   InterpolateScale(start, goal, targetComponent.transform.localScale, scaleInterpolationSpeed));
                    }
                }
            }
        }
